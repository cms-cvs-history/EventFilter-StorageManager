...
namespace edm
{  
  OnlineHttpReader::OnlineHttpReader(edm::ParameterSet const& ps,
  ...
  std::auto_ptr<edm::EventPrincipal> OnlineHttpReader::getOneEvent()
  {
    ...
    do {
      ...
      if(han==0)
      {
        ...
        throw cms::Exception("getOneEvent","OnlineHttpReader")
    ...
    FDEBUG(9) << "OnlineHttpReader received len = " << len << std::endl;
    buf_.resize(len);
    ...
    if (msgView.code() == Header::DONE) {
      ...
      try {
        ...
        if (hdrView.code() != Header::EVENT) {
          throw cms::Exception("OnlineHttpReader", "readOneEvent");
        }
  ...
  void OnlineHttpReader::readHeader()
  {
      ...
      if(han==0)
        {
          ...
          throw cms::Exception("readHeader","OnlineHttpReader")
    ...
    int len = data.d_.length();
    FDEBUG(9) << "OnlineHttpReader received registry len = " << len << std::endl;
    regdata.resize(len);
  ...
  void OnlineHttpReader::registerWithEventServer()
  {
    ...
    do {
      ...
      if(han==0)
        {
          cerr << "could not create handle" << endl;
          throw cms::Exception("registerWithEventServer","OnlineHttpReader")
